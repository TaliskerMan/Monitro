...
# Auto-increment version/build number (optional helper; skipped if absent)
WORKFLOW_TOOLS="${WORKFLOW_TOOLS:-../workflow-tools}"
if [ -f "${WORKFLOW_TOOLS}/increment_build.py" ]; then
    python3 "${WORKFLOW_TOOLS}/increment_build.py" "${SCRIPT_DIR:-.}"
else
    echo "Note: increment_build.py not found under ${WORKFLOW_TOOLS}; skipping version bump."
fi
...
# Single source of truth: propagate the frontend version to the backend so the
# daemon pubspec, the API /health response, and the About screen all agree.
sed -i '' "s/^version: .*/version: $base_version/" backend/pubspec.yaml || true
cat > backend/lib/version.dart <<EOF
// GENERATED by build_macos.sh from the frontend pubspec — do not edit by hand.
const String monitroVersion = '$base_version';
EOF
echo "    Synced backend version constant to $base_version"
...
create-dmg \
  ...
  --background "${WORKFLOW_TOOLS:-../workflow-tools}/macos/dmg_background.png" \
  --window-pos 200 120 \
  --window-size 600 400 \
  --icon-size 120 \
...
if [ ! -f "${DMG_FILE}" ]; then
    ...
    if xcrun notarytool submit "${DMG_FILE}" --keychain-profile "AC_PASSWORD" --wait; then
        xcrun stapler staple "${DMG_FILE}"
                ...
                LEMON_DIR="${MONITRO_DEPLOY_DIR:-./dist}/${APP_NAME_EXTRACTED}${DATE_STR}${VERSION_EXTRACTED}"

                mkdir -p "${LEMON_DIR}"

                cp "${DMG_FILE}" "${LEMON_DIR}/"

                echo "    Copied notarized DMG to deploy dir: ${LEMON_DIR}"
